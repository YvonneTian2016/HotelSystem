...
void CHotel::JudSucpreBook(string name, int age, string wp, string idlicences, string sex,string city, int vipyes,int hour,int day,string roomid ){
  CRoom* p=GetRoom(xb_SearchBylivID(roomid));
...
void CHotel::outdengji(int xb){
   	sitRec<<"登记入住的姓名"<<RoomArray[xb].GetCustomer1()->GetName()<<endl;
    sitRec<<"登记入住的房号"<<RoomArray[xb].GetRoomID()<<endl;
    sitRec<<"登记入住的房间类型"<<RoomArray[xb].GetRoomClass()<<endl;
}
void CHotel::outtuifang(int xb){
sitRec<<"退房的姓名"<<RoomArray[xb].GetCustomer1()->GetName()<<endl;
sitRec<<"退房的房号"<<RoomArray[xb].GetRoomID()<<endl;
sitRec<<"房间金额"<<RoomArray[xb].GetPrice();//获取标准价格;
sitRec<<"房间的总金额"<<topric<<endl;
